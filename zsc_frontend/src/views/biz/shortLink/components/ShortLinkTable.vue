  ...
  <div>
    ...
    <el-table v-loading="loading" :data="linkList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" width="220" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="DataAnalysis" @click="handleStats(scope.row)" v-hasPermi="['biz:shortlink:query']">统计</el-button>
...
function handleStats(row) {
  const linkId = row.linkId || ids.value[0]
  router.push(`/biz/shortlink/stats/${linkId}`)
}
