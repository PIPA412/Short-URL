...
-- 3a. 将系统管理/监控/工具的排序后移，给业务管理腾位置
UPDATE `sys_menu` SET `order_num` = 2 WHERE `menu_id` = 1 AND `menu_name` = '系统管理';
UPDATE `sys_menu` SET `order_num` = 3 WHERE `menu_id` = 2 AND `menu_name` = '系统监控';
UPDATE `sys_menu` SET `order_num` = 4 WHERE `menu_id` = 3 AND `menu_name` = '系统工具';

-- 3b. 添加「业务管理」目录（排在首页下方第一位，order_num = 1）
